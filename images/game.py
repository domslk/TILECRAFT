    ...
    def update(dt):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP:
                ...
                if event.key == pygame.K_e:
                    if not show_crafting_overlay:
                        show_inventory_overlay = not st.show_inventory_overlay
                if event.key == pygame.K_ESCAPE:
                    if show_crafting_overlay:
                        show_crafting_overlay = not st.show_crafting_overlay
                    else:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 3:
                    if not st.show_inventory_overlay and not show_crafting_overlay and not st.pause_screen.show_pause_screen:
                        x, y = event.pos
                ...
                if event.button == 1 and not st.pause_screen.over_button:
                    ...
                    if st.show_inventory_overlay or show_crafting_overlay:
                        mouse_x, mouse_y = event.pos
                        pressed_key = pygame.key.get_pressed()
                        active_overlay = st.crafting_overlay if show_crafting_overlay else st.inventory_overlay
        ...
        if not st.show_inventory_overlay and not show_crafting_overlay and not st.pause_screen.show_pause_screen:
            Block.draw(x_w=st.player.x_w, y_w=st.player.y_w, texture=textures.PLAYER_TEXTURE)
